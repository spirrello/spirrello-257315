...
module "cluster01-pool-01" {
  source         = "github.com/spirrello/terraform-gcp.git//modules/gke-node-pools"
  cluster_name   = module.cluster-01.name
  node_pool_name = "${module.cluster-01.name}-01"
  machine_type   = "n1-standard-4"
  min_node_count = 1
  max_node_count = 2
  region         = var.region
  tags           = [module.cluster-01.name]
}
